...
const state = {
  ...
  sectionPanelState: {screenId: null, discoveryQuestions: false, aiHistory: false},
};
...
function ensureSectionPanelState(screenId) {
  if (state.sectionPanelState.screenId !== screenId) {
    state.sectionPanelState = {screenId, discoveryQuestions:false, aiHistory:false};
  }
  return state.sectionPanelState;
}
function toggleSectionPanel(panelName, sectionId, button) {
  const panels = ensureSectionPanelState(sectionId);
  const panelId = panelName === 'discoveryQuestions' ? 'section-discovery-questions' : 'section-ai-history';
  ...
  if (button) button.textContent = panelName === 'discoveryQuestions'
    ? (show ? 'Hide Discovery Questions' : 'Discovery Questions')
    : (show ? 'Hide AI History' : 'AI History');
}
...
async function showDemoPlan(){if(!navigator.onLine)throw new Error('Demo plan generation requires an online connection.');if(!state.aiStatus?.policy?.allowed)throw new Error(state.aiStatus?.policy?.reason||'AI enhancement is not configured for this environment.');if(!state.report.capability_mappings.some(item=>item.approval_state==='APPROVED'))throw new Error('Approve at least one capability mapping before generating a demo plan.');closeModal();const wrap=document.createElement('div');wrap.id='modal-root';wrap.className='modal-backdrop';wrap.innerHTML=`<section id="ai-demo-plan-modal" class="modal ai-enhancement-modal" role="dialog" aria-modal="true" aria-label="Generate demo preparation plan"><div class="section-head"><div><div class="card-meta"><span class="badge badge-cyan">DEMO ORCHESTRATION</span><span>${esc(state.report.report.title)}</span></div><h2>Create the customer-specific demo flow</h2><p class="help">The plan uses accepted operational content, approved capability mappings, approved benefits, and the report-level demo settings.</p></div><button class="btn btn-ghost btn-small" data-action="close-modal">Close</button></div><div class="ai-comparison-grid"><section class="ai-comparison-panel"><div class="ai-panel-title"><h3>Demo inputs</h3><span class="badge">Internal only</span></div><p><strong>Audience:</strong> ${esc(state.report.demo_settings?.audience||'Not specified')}</p><p><strong>Duration:</strong> ${esc(state.report.demo_settings?.duration_minutes||45)} minutes</p><p><strong>Additional guidance:</strong> ${esc(state.report.demo_settings?.additional_priorities||'None entered')}</p></section><section class="ai-comparison-panel"><div class="ai-panel-title"><h3>AI-proposed demo plan</h3><span class="help">Human approval required</span></div><div id="ai-demo-plan-output"><div class="ai-working"><div class="spinner" aria-hidden="true"></div><p>Preparing demo plan…</p></div></div></section></div></section>`;document.body.appendChild(wrap);await requestDemoPlan();}
...
function reportSectionContent(section) {
  ...
  const panels = ensureSectionPanelState(section.id);
  const historySuggestions = report.ai_suggestions
    .filter(item => item.section_id === section.id && ['OBSERVATION_ENHANCEMENT','SOLUTION_APPROACH','TARGETED_BENEFITS'].includes(item.purpose))
    .slice(0, 20);
  const historyLabel = item => item.review_state === 'APPROVED' ? 'Accepted' : item.review_state === 'REJECTED' ? 'Rejected' : item.review_state === 'PENDING' ? 'Not Reviewed' : item.review_state;
  const historyText = item => item.content?.enhanced_text || item.content?.solution_text || (item.content?.benefits || []).map(value => value.statement).join('\n') || '';
  const canReviewHistory = item => ['OBSERVATION_ENHANCEMENT','TARGETED_BENEFITS'].includes(item.purpose) || canReview(report.access_scope);
  return `
    ...
    <section class="card">
      <div class="section-head"><div><div class="card-meta">${section.process_module?`<span>${esc(section.process_module.replaceAll('_',' '))}</span>`:''}</div><h2>${esc(section.title)}</h2></div><div class="toolbar"><div class="ai-enhance-control"><button class="btn btn-primary btn-small" data-action="ai-enhance-observations" ${state.aiStatus?.policy?.allowed?'':'disabled'} title="${esc(state.aiStatus?.policy?.reason || 'AI status unavailable')}">AI Enhance</button>${(()=>{const aiStatus=aiEnhancementReviewStatus(section.id);return `<small class="ai-enhance-status ${aiStatus.className}" data-ai-enhance-status="${section.id}">Status: ${esc(aiStatus.label)}</small>`;})()}</div><button class="btn btn-ghost btn-small" data-action="toggle-discovery-questions" aria-expanded="${panels.discoveryQuestions?'true':'false'}">${panels.discoveryQuestions?'Hide Discovery Questions':'Discovery Questions'}</button><button class="btn btn-ghost btn-small" data-action="toggle-ai-history" aria-expanded="${panels.aiHistory?'true':'false'}">${panels.aiHistory?'Hide AI History':'AI History'}</button><button class="btn btn-ghost btn-small" data-action="section-version-history">Version history</button>${canOwn(report.access_scope)?'<button class="btn btn-danger btn-small" data-action="remove-section">Remove</button>':''}</div></div>
    ...
    <section class="card transient-section-panel ${panels.discoveryQuestions?'':'hidden'}" id="section-discovery-questions">
      <div class="section-head"><div><h2>Discovery Questions</h2><p class="help">Reference questions only. Responses are no longer entered or collected on operational section pages.</p></div></div>
      <div class="prompt-list">${prompts.length ? prompts.map(p => `<article class="prompt-card discovery-question-reference"><div class="prompt-question"><span>${esc(p.question)}</span></div></article>`).join('') : '<p class="help">No discovery questions are configured for this operational area.</p>'}</div>
    </section>
    <section class="card transient-section-panel ${panels.aiHistory?'':'hidden'}" id="section-ai-history">
      <div class="section-head"><div><h2>AI History</h2><p class="help">Previous AI activity for this operational section. Review state and generated wording remain available without occupying the normal working page.</p></div></div>
      <div class="ai-history-list">${historySuggestions.length ? historySuggestions.map(item => { const text=historyText(item); const label=historyLabel(item); return `<article class="finding"><div class="section-head"><div><div class="card-meta"><span>${item.purpose==='SOLUTION_APPROACH'?'Cloud Inventory approach':item.purpose==='TARGETED_BENEFITS'?'Targeted benefits':'Current Operations enhancement'}</span><span>${esc(fmtDateTime(item.created_at))}</span></div></div><span class="badge ${item.review_state==='APPROVED'?'badge-success':item.review_state==='REJECTED'?'badge-danger':'badge-warning'}">${esc(label)}</span></div>${text?`<p class="ai-history-text">${esc(text).replaceAll('\n','<br>')}</p>`:'<p class="help">No generated wording was stored for this AI event.</p>'}${item.review_state==='PENDING'&&canReviewHistory(item)?`<div class="card-actions"><button class="btn btn-primary btn-small" data-action="review-ai" data-id="${item.id}" data-decision="APPROVED">Accept</button><button class="btn btn-ghost btn-small" data-action="review-ai" data-id="${item.id}" data-decision="REJECTED">Reject</button></div>`:''}</article>`; }).join('') : '<p class="help">AI has not generated any history for this section.</p>'}</div>
    </section>
...
function overviewContent() {
  ...
  return `
    ...
    <section class="card report-governance-card" id="report-readiness">
      ...
      <div class="table-wrap readiness-table"><table><thead><tr><th>Operational section</th><th>Status</th><th>Current Ops</th><th>CI Approach</th><th>Mappings</th><th>Benefits</th><th>Action</th></tr></thead><tbody>${readiness.sections.map(item=>`<tr><td><strong>${esc(item.title)}</strong>${item.missing?.length?`<br><span class="help">Missing: ${esc(item.missing.join(', '))}</span>`:''}</td><td>${readinessBadge(item.status)}</td><td>${item.current_operations?'✓':'—'}</td><td>${item.approach_present?'✓':'—'}</td><td>${esc(item.approved_mapping_count)}${item.pending_mapping_count?` <span class="badge badge-warning">${item.pending_mapping_count} pending</span>`:''}</td><td>${esc(item.approved_benefit_count)}${item.pending_benefit_count?` <span class="badge badge-warning">${item.pending_benefit_count} pending</span>`:''}</td><td><button class="btn btn-ghost btn-small" data-action="navigate-quality-section" data-section-id="${item.section_id}">Open</button></td></tr>`).join('')}</tbody></table></div>
...
async function renderReport(id, sectionId = null) {
  ...
  ensureSectionPanelState(screenId);
...
async function handleClick(event) {
  ...
  try {
    ...
    if(action==='toggle-discovery-questions'){toggleSectionPanel('discoveryQuestions', section.id, target);return;}
    if(action==='toggle-ai-history'){toggleSectionPanel('aiHistory', section.id, target);return;}
